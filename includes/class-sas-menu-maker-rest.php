<?php
/**
 * SAS Menu Maker REST controller.
 *
 ...
 * REST controller.
 */
class SAS_Menu_Maker_REST {

	/**
	 * Namespace for all SAS Menu Maker REST routes.
	 */
	...
	private static $term_resources = array(
		'categories' => array(
			'repo'        => 'SAS_Menu_Maker_Category_Repository',
			'slug_prefix' => 'categories',
			'label'       => 'category',
		),
		'tags'       => array(
			'repo'        => 'SAS_Menu_Maker_Tag_Repository',
			'slug_prefix' => 'tags',
			'label'       => 'tag',
		),
	);
	...
	public static function list_options() {
		...
		foreach ( self::editable_options() as $key => $spec ) {
			$out[ $key ] = SAS_Menu_Maker_Options::get( $key, $spec['default'] );
		}
	...
	public static function save_options( WP_REST_Request $request ) {
		...
		foreach ( $editable as $key => $spec ) {
			...
			SAS_Menu_Maker_Options::update( $key, $sanitized );
		}
	...
	/**
	 * Only admins may write SAS Menu Maker data (for now).
	 *
	 * @return bool
	 */
	...
	private static function handle_create( WP_REST_Request $request, $resource ) {
		...
		if ( '' === $name ) {
			return new WP_Error( 'sas_menu_maker_invalid_name', __( 'Name is required.', 'sas-menu-maker' ), array( 'status' => 400 ) );
		}
		...
		$slug = SAS_Menu_Maker_Slug::generate(
			$config['slug_prefix'],
			$name,
		...
		if ( null === $entity ) {
			return new WP_Error( 'sas_menu_maker_insert_failed', __( 'Could not save.', 'sas-menu-maker' ), array( 'status' => 500 ) );
		}
	...
	private static function handle_update( WP_REST_Request $request, $resource ) {
		...
		if ( $request->has_param( 'name' ) ) {
			...
			$data['slug'] = SAS_Menu_Maker_Slug::generate(
				$config['slug_prefix'],
				$name,
		...
		if ( null === $updated ) {
			return new WP_Error( 'sas_menu_maker_update_failed', __( 'Could not update.', 'sas-menu-maker' ), array( 'status' => 500 ) );
		}
	...
	private static function handle_delete( WP_REST_Request $request, $resource ) {
		...
		if ( ! $ok ) {
			return new WP_Error( 'sas_menu_maker_delete_failed', __( 'Could not delete.', 'sas-menu-maker' ), array( 'status' => 500 ) );
		}
	...
	private static function validate_relations( WP_REST_Request $request, $repo, $self_id ) {
		...
		if ( $request->has_param( 'media_id' ) ) {
			...
			if ( $media_id > 0 && ! wp_attachment_is_image( $media_id ) ) {
				return new WP_Error( 'sas_menu_maker_invalid_media', __( 'Selected media is not an image.', 'sas-menu-maker' ), array( 'status' => 400 ) );
			}
		}
	...
	private static function not_found( $resource ) {
		unset( $resource );
		return new WP_Error( 'sas_menu_maker_not_found', __( 'Not found.', 'sas-menu-maker' ), array( 'status' => 404 ) );
	}
	...
	public static function list_allergens() {
		$rows = SAS_Menu_Maker_Allergen_Repository::all();
		return new WP_REST_Response( $rows, 200 );
	}
	...
	public static function get_allergen( WP_REST_Request $request ) {
		$entity = SAS_Menu_Maker_Allergen_Repository::find( (int) $request->get_param( 'id' ) );
		if ( null === $entity ) {
	...
	public static function create_allergen( WP_REST_Request $request ) {
		...
		if ( '' === $code ) {
			return new WP_Error( 'sas_menu_maker_invalid_code', __( 'Code is required.', 'sas-menu-maker' ), array( 'status' => 400 ) );
		}
		if ( '' === $name ) {
			return new WP_Error( 'sas_menu_maker_invalid_name', __( 'Name is required.', 'sas-menu-maker' ), array( 'status' => 400 ) );
		}
		if ( SAS_Menu_Maker_Allergen_Repository::code_exists( $code ) ) {
			return new WP_Error( 'sas_menu_maker_duplicate_code', __( 'This code is already in use.', 'sas-menu-maker' ), array( 'status' => 409 ) );
		}

		$entity = SAS_Menu_Maker_Allergen_Repository::insert(
			array(
				'code'        => $code,
				'name'        => $name,
				'description' => (string) $request->get_param( 'description' ),
	...
	public static function update_allergen( WP_REST_Request $request ) {
		$id       = (int) $request->get_param( 'id' );
		$existing = SAS_Menu_Maker_Allergen_Repository::find( $id );
		if ( null === $existing ) {
			return self::not_found( 'allergen' );
		}
		...
		if ( $request->has_param( 'code' ) ) {
			...
			if ( SAS_Menu_Maker_Allergen_Repository::code_exists( $code, $id ) ) {
				return new WP_Error( 'sas_menu_maker_duplicate_code', __( 'This code is already in use.', 'sas-menu-maker' ), array( 'status' => 409 ) );
			}
			$data['code'] = $code;
		}
		...
		$updated = SAS_Menu_Maker_Allergen_Repository::update( $id, $data );
		if ( null === $updated ) {
			return new WP_Error( 'sas_menu_maker_update_failed', __( 'Could not update.', 'sas-menu-maker' ), array( 'status' => 500 ) );
		}
	...
	public static function delete_allergen( WP_REST_Request $request ) {
		...
		$ok = SAS_Menu_Maker_Allergen_Repository::delete( $id );
		if ( ! $ok ) {
			return new WP_Error( 'sas_menu_maker_delete_failed', __( 'Could not delete.', 'sas-menu-maker' ), array( 'status' => 500 ) );
		}
	...
	public static function list_items() {
		$rows = SAS_Menu_Maker_Item_Repository::all();
		$rows = array_map( array( __CLASS__, 'present' ), $rows );
		return new WP_REST_Response( $rows, 200 );
	}
	...
	public static function get_item( WP_REST_Request $request ) {
		$id     = (int) $request->get_param( 'id' );
		$entity = SAS_Menu_Maker_Item_Repository::find( $id );
		if ( null === $entity ) {
	...
	public static function create_item( WP_REST_Request $request ) {
		...
		$slug = SAS_Menu_Maker_Slug::generate(
			'items',
			$name,
			array( 'SAS_Menu_Maker_Item_Repository', 'slug_exists' )
		);

		$entity = SAS_Menu_Maker_Item_Repository::insert(
			array(
				'name'              => $name,
				'slug'              => $slug,
	...
	public static function update_item( WP_REST_Request $request ) {
		$id       = (int) $request->get_param( 'id' );
		$existing = SAS_Menu_Maker_Item_Repository::find( $id );
		if ( null === $existing ) {
			return self::not_found( 'item' );
		}
		...
		if ( $request->has_param( 'name' ) ) {
			...
			$data['slug'] = SAS_Menu_Maker_Slug::generate(
				...
				function ( $candidate ) use ( $id ) {
					return SAS_Menu_Maker_Item_Repository::slug_exists( $candidate, $id );
				}
			);
		}
		...
		$updated = SAS_Menu_Maker_Item_Repository::update( $id, $data );
		if ( null === $updated ) {
			return new WP_Error( 'sas_menu_maker_update_failed', __( 'Could not update.', 'sas-menu-maker' ), array( 'status' => 500 ) );
		}
	...
	public static function delete_item( WP_REST_Request $request ) {
		...
		$blocking = SAS_Menu_Maker_Offer_Repository::offers_using_item( $id );
		if ( ! empty( $blocking ) ) {
			return new WP_Error(
				'sas_menu_maker_item_in_offer',
				sprintf(
		...
		$ok = SAS_Menu_Maker_Item_Repository::delete( $id );
		if ( ! $ok ) {
			return new WP_Error( 'sas_menu_maker_delete_failed', __( 'Could not delete.', 'sas-menu-maker' ), array( 'status' => 500 ) );
		}
	...
	public static function bulk_edit_items( WP_REST_Request $request ) {
		...
		if ( empty( $item_ids ) ) {
			return new WP_Error( 'sas_menu_maker_no_items', __( 'No items selected.', 'sas-menu-maker' ), array( 'status' => 400 ) );
		}
		...
		$updated = SAS_Menu_Maker_Item_Repository::bulk_edit( $item_ids, $sanitized );
	...
	private static function validate_bulk_operations( array $operations ) {
		$relation_specs = array(
			'categories' => array( 'SAS_Menu_Maker_Category_Repository', array( 'replace', 'add', 'remove' ), __( 'Unknown category.', 'sas-menu-maker' ) ),
			'tags'       => array( 'SAS_Menu_Maker_Tag_Repository', array( 'replace', 'add', 'remove' ), __( 'Unknown tag.', 'sas-menu-maker' ) ),
			'allergens'  => array( 'SAS_Menu_Maker_Allergen_Repository', array( 'replace', 'add', 'remove' ), __( 'Unknown allergen.', 'sas-menu-maker' ) ),
		);
		...
		foreach ( $relation_specs as $key => $spec ) {
			...
			if ( ! in_array( $mode, $spec[1], true ) ) {
				return new WP_Error( 'sas_menu_maker_invalid_mode', __( 'Invalid mode for a relation operation.', 'sas-menu-maker' ), array( 'status' => 400 ) );
			}
			...
			foreach ( $ids as $raw_id ) {
				...
				if ( null === call_user_func( array( $spec[0], 'find' ), $id ) ) {
					return new WP_Error( 'sas_menu_maker_invalid_relation', $spec[2], array( 'status' => 400 ) );
				}
			}
		}
		...
		if ( ! empty( $operations['base_price'] ) ) {
			...
			if ( ! in_array( $mode, array( 'replace', 'increase', 'decrease' ), true ) ) {
				return new WP_Error( 'sas_menu_maker_invalid_mode', __( 'Invalid base-price mode.', 'sas-menu-maker' ), array( 'status' => 400 ) );
			}
		}
		...
		if ( ! empty( $operations['variant_prices'] ) ) {
			...
			if ( ! in_array( $mode, array( 'increase', 'decrease' ), true ) ) {
				return new WP_Error( 'sas_menu_maker_invalid_mode', __( 'Invalid variant-price mode.', 'sas-menu-maker' ), array( 'status' => 400 ) );
			}
		}
	...
	private static function validate_item_relations( WP_REST_Request $request ) {
		...
		$relation_specs = array(
			'category_ids' => array( 'SAS_Menu_Maker_Category_Repository', __( 'Unknown category.', 'sas-menu-maker' ) ),
			'tag_ids'      => array( 'SAS_Menu_Maker_Tag_Repository', __( 'Unknown tag.', 'sas-menu-maker' ) ),
			'allergen_ids' => array( 'SAS_Menu_Maker_Allergen_Repository', __( 'Unknown allergen.', 'sas-menu-maker' ) ),
		);
		...
		foreach ( $relation_specs as $param => $spec ) {
			...
			foreach ( $ids as $id ) {
				...
				if ( null === call_user_func( array( $spec[0], 'find' ), $int_id ) ) {
					return new WP_Error( 'sas_menu_maker_invalid_relation', $spec[1], array( 'status' => 400 ) );
				}
			}
		}
	...
	private static function variants_blocked_by_offers( $item_id, array $variants ) {
		$to_remove = SAS_Menu_Maker_Item_Repository::variants_to_remove( $item_id, $variants );
		foreach ( $to_remove as $vid ) {
			$blocking = SAS_Menu_Maker_Offer_Repository::offers_using_variant( $vid );
			if ( ! empty( $blocking ) ) {
				return new WP_Error(
					'sas_menu_maker_variant_in_offer',
					sprintf(
						/* translators: %s: comma-separated offer names */
	...
	public static function list_offers() {
		$rows = SAS_Menu_Maker_Offer_Repository::all();
		$rows = array_map( array( __CLASS__, 'present' ), $rows );
		return new WP_REST_Response( $rows, 200 );
	}
	...
	public static function get_offer( WP_REST_Request $request ) {
		$id     = (int) $request->get_param( 'id' );
		$entity = SAS_Menu_Maker_Offer_Repository::find( $id );
		if ( null === $entity ) {
	...
	public static function create_offer( WP_REST_Request $request ) {
		...
		$slug = SAS_Menu_Maker_Slug::generate(
			'offers',
			$name,
			array( 'SAS_Menu_Maker_Offer_Repository', 'slug_exists' )
		);

		$entity = SAS_Menu_Maker_Offer_Repository::insert(
			array(
				'name'            => $name,
				'slug'            => $slug,
				'description'     => (string) $request->get_param( 'description' ),
	...
	public static function update_offer( WP_REST_Request $request ) {
		$id       = (int) $request->get_param( 'id' );
		$existing = SAS_Menu_Maker_Offer_Repository::find( $id );
		if ( null === $existing ) {
			return self::not_found( 'offer' );
		}
		...
		if ( $request->has_param( 'name' ) ) {
			...
			$data['slug'] = SAS_Menu_Maker_Slug::generate(
				...
				$name,
				function ( $candidate ) use ( $id ) {
					return SAS_Menu_Maker_Offer_Repository::slug_exists( $candidate, $id );
				}
			);
		}
		...
		$updated = SAS_Menu_Maker_Offer_Repository::update( $id, $data );
		if ( null === $updated ) {
			return new WP_Error( 'sas_menu_maker_update_failed', __( 'Could not update.', 'sas-menu-maker' ), array( 'status' => 500 ) );
		}
	...
	public static function delete_offer( WP_REST_Request $request ) {
		...
		$ok = SAS_Menu_Maker_Offer_Repository::delete( $id );
		if ( ! $ok ) {
			return new WP_Error( 'sas_menu_maker_delete_failed', __( 'Could not delete.', 'sas-menu-maker' ), array( 'status' => 500 ) );
		}
	...
	private static function validate_offer_payload( WP_REST_Request $request, array $items ) {
		...
		if ( $from && $until && strtotime( $until ) < strtotime( $from ) ) {
			return new WP_Error( 'sas_menu_maker_invalid_dates', __( 'Valid-until must be on or after valid-from.', 'sas-menu-maker' ), array( 'status' => 400 ) );
		}

		foreach ( $items as $line ) {
			$item_id = isset( $line['item_id'] ) ? (int) $line['item_id'] : 0;
			if ( $item_id <= 0 ) {
				return new WP_Error( 'sas_menu_maker_invalid_line', __( 'Each offer line needs an item.', 'sas-menu-maker' ), array( 'status' => 400 ) );
			}

			$item = SAS_Menu_Maker_Item_Repository::find( $item_id );
			if ( null === $item ) {
				return new WP_Error( 'sas_menu_maker_unknown_item', __( 'Unknown item in offer.', 'sas-menu-maker' ), array( 'status' => 400 ) );
			}
			...
			if ( $has_variants && $variant_id <= 0 ) {
				return new WP_Error(
					'sas_menu_maker_variant_required',
					sprintf(
			...
			if ( $variant_id > 0 && ! isset( $valid_variants[ $variant_id ] ) ) {
				return new WP_Error(
					'sas_menu_maker_variant_mismatch',
					sprintf(
						/* translators: %s: item name */
			...
			if ( isset( $line['quantity'] ) && (int) $line['quantity'] < 1 ) {
				return new WP_Error( 'sas_menu_maker_invalid_quantity', __( 'Quantity must be at least 1.', 'sas-menu-maker' ), array( 'status' => 400 ) );
			}
		}
